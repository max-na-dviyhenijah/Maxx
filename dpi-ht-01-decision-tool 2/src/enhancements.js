...
export function enhanceCase(data, review) {
  ...
  document.querySelectorAll('.decision-body span,.review-columns span').forEach(el=>{
    if(el.textContent==='Certified answer') el.textContent='Final answer';
  ...
  if(!review){
    ...
    search.addEventListener('input',()=>queueMicrotask(updateCount));document.querySelectorAll('.filter').forEach(b=>b.addEventListener('click',()=>queueMicrotask(updateCount)));updateCount();
  }else{
    document.querySelector('.review-score span').textContent='material judgments reviewed and certified';
    const flags=document.querySelectorAll('.review-flags article');
    flags[1].innerHTML='<span>Revised after challenge</span><strong>2</strong><small>D058 and D072 follow the second analysis</small>';
    ...
    document.querySelectorAll('.review-card').forEach(card=>{card.querySelector('.final span').textContent='Final certified answer';});
