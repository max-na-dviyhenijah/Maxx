...
const operational = {
  ...
  D013: ["Reconstruct the EUR 100,000 Event Things payment as EUR 55,000 for current purchases and EUR 45,000 for opening payables, assuming complete purchases, payments and closing balances with no credits, advances or other adjustments. Current purchases of EUR 114,000 less confirmed closing payable of EUR 59,000 imply EUR 55,000 paid against current purchases. The residual EUR 45,000 is inferred opening debt, not a directly documented allocation. Obtain the supplier ledger to confirm.", ["E02","E06"], "medium"],
...
const material = {
  ...
  D091: { answer: "Use the reconstructed accounts as a provisional basis for board analysis. Defer final valuation approval until the inventory conflict, insurance completeness, depreciation workings and opening-balance support have been resolved or explicitly addressed in valuation terms.", evidence: ["E00","E02","E05","E08","E11"], confidence: "medium", aiProposal: "Approve the corrected reconstruction with explicit uncertainty disclosures.", independentChallenge: "Delay approval until insurance completeness, inventory valuation and opening-balance support are resolved.", reasoning: "The statements reconcile under disclosed assumptions. Inventory has a EUR 9,000 alternative and the legal estimate has a EUR 20,000–EUR 30,000 range. Insurance and the uncertainty around depreciation cannot be quantified from the supplied evidence. A balanced reconstruction does not establish a final verified valuation basis.", effect: [null,null,null,null,null], changed: true },
  D100: { answer: "Reject the EUR 312,000 management profit for earn-out. EUR 65,000 is provisional reconstructed profit under the stated assumptions, not finally verified earnings. Resolve the evidence gaps and agree the earnings definition before approving an earn-out payment.", evidence: ["E01","E02","E04","E05","E08","E09","E10","E11"], confidence: "high", aiProposal: "Exclude the management claim from valuation and earn-out calculations.", independentChallenge: "A normalized EBITDA metric could differ from statutory profit, but it must be rebuilt from verified accounts rather than the unsupported deck.", reasoning: "High confidence applies to rejecting the unsupported management claim. The EUR 65,000 point estimate remains conditional on inventory, insurance, depreciation and opening balances. Their unresolved effects must be addressed before an earn-out is finalized.", effect: [-247000,0,0,0,-247000], changed: false }
};
...
const decisions = template.decisions.map((d) => {
  ...
  return {
    ...
    reviewOutcome: d.id === 'D091' ? 'revised_after_external_feedback_confirmed_by_student' : m.changed ? "revised_after_independent_review" : "agreement_with_qualifications_where_noted",
...
const submission = {
  ...
  certification: { status: "certified_by_student", studentIdentityConfirmed: true, identityBasis: "Name and ID supplied by the student in this conversation; no institutional identity verification performed.", note: "On 2026-09-20 the student confirmed having read and agreed with the current version, including the clarifications to D013, D091 and D100. This records student approval, not independent verification of missing evidence." },
  evidence,
  decisions,
  schedules: {
...
submission.aiReviewProvenance={method:'Agent 2 was launched with fork_turns=none and instructed to read only original evidence, without Agent 1 conclusions.',source:'Recorded subagent exchange in the originating conversation',recordUrl:'/AI-REVIEW-RECORD.md',limitation:'The linked record is an excerpt and summary assembled from the conversation, not a complete platform transcript export.'};
